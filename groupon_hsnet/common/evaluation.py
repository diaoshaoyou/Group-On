...
class Evaluator:
    ...
    @classmethod
    def classify_prediction(cls, pred_mask, batch, q_id=-1, s_id=-1):
        if q_id>=0 and s_id==-1:
            gt_masks = batch.get('query_masks') # shape=(b, N_q, h, w)
            gt_mask=gt_masks[:,q_id]
        elif s_id>=0 and q_id==-1:  # alignment
            gt_masks = batch.get('support_masks') # shape=(b, nshot, h, w)
            gt_mask = gt_masks[:, s_id]

        # Apply ignore_index in PASCAL-5i masks (following evaluation scheme in PFE-Net (TPAMI 2020))
        query_ignore_idxs = batch.get('query_ignore_idxs')
        if query_ignore_idxs is not None:
            query_ignore_idx=query_ignore_idxs[:,q_id]
