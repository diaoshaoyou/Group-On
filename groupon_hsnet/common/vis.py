...
class Visualizer:
    ...
    def initialize(cls, visualize):
        ...
        cls.vis_path = 'groupon_hsnet/vis/'
        if os.path.exists(cls.vis_path): 
            shutil.rmtree(cls.vis_path)
        os.makedirs(cls.vis_path)

    @classmethod
    def visualize_prediction_batch(cls, spt_img_b, spt_mask_b, qry_imgs, qry_masks, pred_mask_b, cls_id_b, batch_idx, iou_b=None):
        spt_img_b = utils.to_cpu(spt_img_b)
        spt_mask_b = utils.to_cpu(spt_mask_b)
        qry_img_b = utils.to_cpu(qry_imgs[:,0])
        qry_mask_b = utils.to_cpu(qry_masks[:,0])
        pred_mask_b = utils.to_cpu(pred_mask_b)
        cls_id_b = utils.to_cpu(cls_id_b)
        supportive_b = utils.to_cpu(qry_imgs[:,1:])

        for sample_idx, (spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id, supportive) in \
                enumerate(zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b, supportive_b)):
            iou = iou_b[sample_idx] if iou_b is not None else None
            cls.visualize_prediction(spt_img, spt_mask, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, supportive, True, iou)
    ...
    @classmethod
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx, sample_idx, supportives, label, iou=None):
        ...
        supportive_imgs = [cls.to_numpy(sp, 'img') for sp in supportives]
        supportive_pils = [cls.to_pil(sp) for sp in supportive_imgs]
        ...
        spt_masked_pils[0].save(cls.vis_path+'%d_%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + '_s.jpg')
        qry_masked_pil.save(cls.vis_path+'%d_%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + '_q.jpg')
        pred_masked_pil.save(cls.vis_path+'%d_%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + '_m.jpg')
        for i, sp_pil in enumerate(supportive_pils):
            sp_pil.save(cls.vis_path+'%d_%d_class-%d_iou-%.2f' % (batch_idx, sample_idx, cls_id, iou) + f'_sp{i}.jpg')
