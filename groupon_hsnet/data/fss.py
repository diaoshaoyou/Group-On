...
class DatasetFSS(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, N_q):
        ...
        with open('groupon_hsnet/data/splits/fss/%s.txt' % split, 'r') as f:
    ...
    def __getitem__(self, idx):
        ...
        query_imgs, query_masks, support_imgs, support_masks = self.load_frame(query_names, support_names)
        ...
        query_masks_tmp = []
        for qmask in query_masks:
            qmask = F.interpolate(qmask.unsqueeze(0).unsqueeze(0).float(), query_imgs.size()[-2:], mode='nearest').squeeze()
            query_masks_tmp.append(qmask)
        query_masks = torch.stack(query_masks_tmp)
    ...
    def load_frame(self, query_names, support_names):
        query_imgs = [Image.open(name).convert('RGB') for name in query_names] 
        support_imgs = [Image.open(name).convert('RGB') for name in support_names]

        query_ids = [name.split('/')[-1].split('.')[0] for name in query_names]
        query_names = [os.path.join(os.path.dirname(name), qid) + '.png' for name, qid in zip(query_names, query_ids)]
        ...
        return query_imgs, query_masks, support_imgs, support_masks
    ...
    def sample_episode(self, idx):
        q_name1 = self.img_metadata[idx]
        class_sample = self.categories.index(q_name1.split('/')[-2])
        ...
        while True:
            ...
            q_name=np.random.choice(range(1, 11), 1, replace=False)[0]
            q_name=os.path.join(os.path.dirname(q_name1), str(q_name)) + '.jpg'
        ...
        while True:  # keep sampling support set if query == support
            ...
            support_name = os.path.join(os.path.dirname(q_name1), str(support_name)) + '.jpg'
    ...
    def build_img_metadata(self):
        ...
        return img_metadata # [img_name1, img_name2, ...]
